...
namespace SDKTemplate
{
    public sealed partial class Scenario_ContinuousDictation : Page
    {
        ...
        // File Selected by picker to transcribe
        private StorageFolder selectedFolder;

        //private MediaElement playbackMediaElement;
        ...
        private IEnumerator<StorageFile> fileListEnumerator;

        private MediaPlayer mediaPlayer;

        public Scenario_ContinuousDictation()
        {
            //playbackMediaElement = new MediaElement();
            ...
            InitializeAudioComponentsAsync();
        }
        ...
        void InitializePropertyValues()
        {
            ...
            mediaPlayer.PlaybackRate = (double)speedRatioSlider.Value;

        }

        async Task InitializeAudioComponentsAsync()
        {
            ...
            foreach (var device in outputDevices)
            {
                ...
                cbAudioOutputSelector.Items.Add(deviceItem);
            }
        }

        private void cbAudioOutputSelector_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            DeviceInformation selectedDevice = (DeviceInformation)((ComboBoxItem)cbAudioOutputSelector.SelectedItem).Tag;
            ...
            if (isListening == false)
            {
                ...
                if (speechRecognizer.State == SpeechRecognizerState.Idle)
                {
                    ...
                    try
                    {
                        ...
                        {
                            IReadOnlyList<StorageFile> fileList = await selectedFolder.GetFilesAsync();
                            IEnumerator<StorageFile> fileListEnumerator = fileList.GetEnumerator();
                            this.fileListEnumerator = fileListEnumerator;
        ...
        private async void PlayMediaFromFolder()
        {
            while (this.fileListEnumerator.MoveNext() && isPlaying)
            {
                StorageFile file = fileListEnumerator.Current;
                textBlockFileSelected.Text = "Currently playing file: " + file.Name;
                var stream = await file.OpenAsync(Windows.Storage.FileAccessMode.Read);
                //playbackMediaElement.SetSource(stream, file.ContentType);
                mediaPlayer.Source = MediaSource.CreateFromStream(stream,"wav");

                //playbackMediaElement.Play();
                mediaPlayer.Play();
                textBlockFileSelected.Text = "About to wait: " + file.Name;
                double timeToWait = (5 / mediaPlayer.PlaybackSession.PlaybackRate);
                await Task.Delay(TimeSpan.FromSeconds(0.5 + timeToWait));
                textBlockFileSelected.Text = "Finished wait: " + file.Name;
            }
        }
        ...
        private void ChangeMediaSpeedRatio(object sender, RangeBaseValueChangedEventArgs e)
        {
            ...
            {
                //playbackMediaElement.DefaultPlaybackRate = (slider.Value / 100);
                mediaPlayer.PlaybackSession.PlaybackRate = (slider.Value / 100);
            }
        }
        ...
        private void ChangeMediaVolume(object sender, RangeBaseValueChangedEventArgs e)
        {
            ...
            if (slider != null && mediaPlayer != null)
            {
                //playbackMediaElement.Volume = slider.Value;
